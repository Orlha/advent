...
const auto size = sizeof(numbers) / sizeof(numbers[0]);

auto calculate_1() {
	for(int i = 0; i < size; ++i) {
		for(int k = i + 1; k < size; ++k) {
			if(numbers[i] + numbers[k] == 2020) return numbers[i] * numbers[k];
		}
	}
	return 0;
}
...
auto calculate_2() {
	for(int i = 0; i < size; ++i) {
		for(int k = i + 1; k < size; ++k) {
			if(numbers[i] + numbers[k] > 2020) continue;
			for(int j = k + 1; j < size; ++j) {
				if(numbers[i] + numbers[k] + numbers[j] == 2020) return numbers[i] * numbers[k] * numbers[j];
			}
		}
	}
	return 0;
...
int main() {
	std::cout << calculate_1() << "\n";
	std::cout << calculate_2() << "\n";
}
